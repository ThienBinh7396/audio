...
<div class="tb-content">
    ...
    <div v-show="!preload">
        <div class="header-background" :style="{'background-image': `url('${topBackground}')`}" />
        ...
        <v-container class="main-container">
            <div class="top-header relative">
                <ul class="top-nav">
                    <li v-for="(item, index) in nav" :key="`nav-${index}`">
                        ...
                        <v-icon size="15" v-if="index < nav.length - 1">mdi-chevron-right</v-icon>
                    </li>
                </ul>
                <v-btn fab x-small color="#000000b3" v-if="!detectMobile" class="top-search" @click="controlShowSearch(true)">
            ...
            <div id="main-content">
                <transition name="list" mode="out-in" @beforeLeave="beforeLeave" @enter="enter" @afterEnter="afterEnter">
                    <router-view />
                </transition>
            </div>
...
export default {
    components: {
        PreloadPage: () =>
            import('@/views/partials/PreloadPage'),
        SearchPartial: () =>
            import('@/views/partials/SearchPartial'),
    },
    data(){
        return {
            preload: true
    ...
    methods: {
        ...
        ...mapActions('app', ['updateResponsiveByWidth', 'initializeIp']),
    ...
    created() {
        ...
        window.imageUrl = function (params) {
    ...
    mounted() {
        let that = this;
        this.$axios.get('https://api.ipify.org')
            .then(rs => {
                let {
                    data
                } = rs;
                that.initializeIp(data);
            })
